...
func getByteCount(users []*User) int {
	maxScore := 0
	for _, user := range users {
	...
	return (bits.Len(uint(maxScore)) + 7) / 8
}

func radixSortBitwise(users []*User) []*User {
	if len(users) == 0 {
		return users
	}

	result := users
	byteCount := getByteCount(users)
	MASK := 0xFF
...
func main() {
	inputFilePrefix := "../data/input/bitwise/users_"
	outputFilePrefix := "../data/output/bitwise/go_sorted_users_"
	fileFormat := ".csv"
	...
	for _, countUsers := range usersInFile {
		...
		sortedUsers := radixSortBitwise(users)
